CREATE OR REPLACE FUNCTION public.award_points_via_nfc_with_reward(
  p_hardware_uid text,
  p_user_id uuid,
  p_expected_merchant_customer_id uuid,
...
BEGIN
  ...
  IF p_expected_merchant_customer_id IS NULL
    OR v_merchant_customer_id IS DISTINCT FROM p_expected_merchant_customer_id THEN
    RETURN (v_award::jsonb || jsonb_build_object(
      'activated_reward_redeemed', false,
      'activated_reward_error', 'Gescannte Karte gehört nicht zur aktivierten Prämie'
    ))::json;
  END IF;

  IF p_activated_visit_number IS NULL
    OR p_activated_reward_label IS NULL
    OR trim(p_activated_reward_label) = '' THEN
    RETURN v_award;
  END IF;
