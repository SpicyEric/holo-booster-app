-- Fix verification card: award 1 point, skip cooldown
UPDATE nfc_chips SET points_value = 1 WHERE lower(stamp_color) IN ('v','verify');
...
BEGIN
  ...
  v_points_to_award := COALESCE(NULLIF(v_nfc_chip.points_value, 0), 1);
  v_is_verify := lower(v_nfc_chip.stamp_color) IN ('v','verify');

  -- 60s cooldown — skip for verification cards
  IF NOT v_is_verify THEN
    ...
    IF v_last_stamp IS NOT NULL THEN
      RETURN json_build_object('success', false, 'error', 'Bitte warte einen Moment, bevor du diesen Stempel erneut verwendest.', 'error_code', 'cooldown', 'cooldown_until', (v_last_stamp + interval '60 seconds'));
    END IF;
  END IF;

  SELECT * INTO v_loyalty_account FROM loyalty_accounts
