...
-- Update the 2-param award_points_via_nfc function to enforce max 3 stamps per merchant per user per day
...
BEGIN
  ...
  -- DAILY LIMIT: Max 3 stamps per merchant per user per day
  SELECT COUNT(*) INTO v_daily_stamp_count
  ...
  WHERE la.user_id = p_user_id
    ...
    AND pt.created_at >= date_trunc('day', now() AT TIME ZONE 'Europe/Berlin') AT TIME ZONE 'Europe/Berlin'
    AND pt.created_at < (date_trunc('day', now() AT TIME ZONE 'Europe/Berlin') + interval '1 day') AT TIME ZONE 'Europe/Berlin';

  IF v_daily_stamp_count >= 3 THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Du hast heute bereits das Maximum von 3 Stempeln bei diesem Geschäft erreicht. Komm morgen wieder!',
...
-- Update the 3-param version too
...
BEGIN
  ...
  IF p_hardware_uid IS NOT NULL AND trim(p_hardware_uid) != '' THEN
    ...
    IF FOUND THEN
      -- Delegate to 2-param version (which has its own cooldown + daily limit)
