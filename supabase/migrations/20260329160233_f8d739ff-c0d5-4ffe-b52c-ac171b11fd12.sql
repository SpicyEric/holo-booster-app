...
-- Update 2-param version: add 60-second per-chip cooldown
...
-- Update 3-param version: add 60-second per-chip cooldown in inline path
...
BEGIN
  ...
  IF p_hardware_uid IS NOT NULL AND trim(p_hardware_uid) != '' THEN
    ...
    IF FOUND THEN
      -- Delegate to 2-param version (which has its own cooldown)
