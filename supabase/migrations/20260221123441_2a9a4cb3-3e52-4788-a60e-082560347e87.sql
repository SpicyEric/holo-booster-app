...
-- Add hardware_uid column to nfc_chips for physical chip verification
ALTER TABLE public.nfc_chips ADD COLUMN IF NOT EXISTS hardware_uid text;

-- Create index for fast lookup during scans
CREATE INDEX IF NOT EXISTS idx_nfc_chips_hardware_uid ON public.nfc_chips (hardware_uid) WHERE hardware_uid IS NOT NULL;

-- Update the award_points_via_nfc function to accept and verify hardware_uid
CREATE OR REPLACE FUNCTION public.award_points_via_nfc(p_chip_data text, p_user_id uuid, p_hardware_uid text DEFAULT NULL)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
