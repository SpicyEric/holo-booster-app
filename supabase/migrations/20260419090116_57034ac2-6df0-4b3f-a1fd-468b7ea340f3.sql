-- 1) Extensions für Scheduled Jobs
CREATE EXTENSION IF NOT EXISTS pg_cron WITH SCHEMA extensions;
CREATE EXTENSION IF NOT EXISTS pg_net WITH SCHEMA extensions;

-- 2) Täglicher Cron-Job: Gekündigte Händler nach Periodenende deaktivieren
DO $$
BEGIN
  -- Bestehenden Job entfernen, falls vorhanden (idempotent)
  PERFORM cron.unschedule('deactivate-expired-customers')
  WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'deactivate-expired-customers'
  );
END$$;
...
-- 3) award_points_via_nfc (2-Parameter-Variante) — active-Check ergänzen
...
-- 4) redeem_message_offer_via_nfc — active-Check ergänzen
...
BEGIN
  ...
  -- NEU: Merchant-Aktivitäts-Check
  SELECT active INTO v_merchant_active
  FROM customers
  WHERE id = v_message.merchant_customer_id;

  IF v_merchant_active IS DISTINCT FROM true THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Dieser Laden ist aktuell nicht aktiv.',
      'error_code', 'merchant_inactive'
