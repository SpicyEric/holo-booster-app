...
DECLARE
  ...
  v_is_v2_merchant boolean;
  v_treat_as_checkin boolean;
...
BEGIN
  ...
  SELECT nc.*, c.company_name, c.name as customer_name, c.active as merchant_active, c.version as merchant_version
  INTO v_nfc_chip
  FROM nfc_chips nc
  ...
  v_is_v2_merchant := COALESCE(v_nfc_chip.merchant_version, '') = 'v2';
  -- V2-Händler: jeder Scan ist ein Check-in (Fullscreen-Overlay zeigen)
  v_treat_as_checkin := v_is_verify_checkin OR v_is_v2_merchant;
  ...
  -- V2: Tageslimit (max 1 Check-in pro Tag pro Händler) prüfen
  IF v_is_v2_merchant THEN
    SELECT pt.created_at INTO v_today_checkin
    FROM point_transactions pt
    JOIN loyalty_accounts la ON la.id = pt.loyalty_account_id
    WHERE la.user_id = p_user_id
      AND pt.merchant_customer_id = v_merchant_customer_id
      AND pt.transaction_type IN ('nfc_stamp', 'check_in')
      AND pt.created_at::date = (now() AT TIME ZONE 'Europe/Berlin')::date
    LIMIT 1;

    IF v_today_checkin IS NOT NULL THEN
      RETURN json_build_object(
        'success', false,
        'error', 'Du hast bei diesem Geschäft heute schon eingecheckt. Komm morgen wieder.',
        'error_code', 'already_checked_in_today'
      );
    END IF;
  END IF;

  SELECT la.* INTO v_loyalty_account
  FROM loyalty_accounts la
  WHERE la.user_id = p_user_id
    AND la.merchant_customer_id = v_merchant_customer_id;

  IF NOT FOUND THEN
    INSERT INTO loyalty_accounts (user_id, merchant_customer_id, points_balance)
    VALUES (p_user_id, v_merchant_customer_id, 0)
  ...
  SELECT NOT EXISTS (
    ...
    WHERE loyalty_account_id = v_loyalty_account.id
      ...
      AND transaction_type IN ('nfc_stamp', 'check_in')
  ...
  -- Erster Check-in: Visit-1-Prämie (sofern platziert) automatisch einlösen
  ...
  RETURN json_build_object(
    ...
    'is_check_in', v_treat_as_checkin,
