...
BEGIN
  ...
  -- Falls dieselbe Hardware-UID schon irgendwo gespeichert wurde, diesen Eintrag wiederverwenden.
  IF v_hardware_uid IS NOT NULL THEN
    SELECT nc.id, nc.merchant_customer_id
    INTO v_existing_id, v_existing_merchant_id
  ...
  -- Sonst nach Karte-ID + Farbe suchen.
  IF v_existing_id IS NULL THEN
    SELECT nc.id, nc.merchant_customer_id
    INTO v_existing_id, v_existing_merchant_id
    FROM public.nfc_chips nc
    WHERE upper(nc.chip_uid) = v_chip_uid
      AND lower(nc.stamp_color) = lower(v_stamp_color)
    ORDER BY nc.created_at DESC
    LIMIT 1;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.register_box_nfc_chip(text, text, text, text, integer) TO authenticated;