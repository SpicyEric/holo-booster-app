-- 1. Daily check-in limit (außer Backstube König) + 2. RPC zum Einlösen der aktivierten V2-Prämie
...
BEGIN
  ...
  IF NOT v_is_verify_checkin THEN
    ...
    -- Tageslimit: 1 Check-in pro Tag pro Merchant.
    -- Ausnahme: Backstube König (e828d21a-...) erlaubt mehrere Check-ins pro Tag.
...
-- Neue RPC: aktivierte V2-Prämie serverseitig als eingelöst markieren.
...
BEGIN
  ...
  RETURN json_build_object('success', true, 'visit_number', p_visit_number, 'label', p_reward_label);
END;
$$;