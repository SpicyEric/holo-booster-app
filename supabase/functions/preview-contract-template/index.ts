...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!roleData || roleData.length === 0) {
      throw new Error("Nur Admins dürfen die Vertragsvorlage herunterladen");
    }
    ...
    return new Response(pdfBytes, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="Vertragsvorlage_${version}.pdf"`,
      },
    });
  } catch (e: any) {
...
function generateContractPdf(d: ContractData): Uint8Array {
  ...
  doc.text("— VORLAGE / MUSTERVERTRAG —", pw / 2, y, { align: "center" });
  doc.setTextColor(0);
  y += 8;
